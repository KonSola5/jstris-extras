export function initLayoutChanges() {
  ...
  document.getElementById("main").insertBefore(mstage, document.getElementById("rstage"));

  let statsDiv = document.querySelector("#main > div:not([class]):not([id])");
  mstage.append(...statsDiv.children);

  // document.getElementsByClassName("comboclass")[0].classList.add("hidden");
  document.querySelector(".chatInputC .emSel").classList.add("hidden");
  ...
  */
  ...
  Slot.prototype.init = function () {
    ...
    if (this.gs.slotStats && this.gs.matrixWidth >= 50) {
      ...
      let shouldDisplayWinCounter = 2 * totalWidth < 0.85 * this.gs.matrixWidth || totalWidth > 0.6 * this.gs.matrixWidth;
  ...
  GameCaption.prototype.speedWarning = function (PPSLimit) {
    if (!(this.GAME_PLACE in this.captions) || !this.captions[this.GAME_PLACE].classList.contains("hidden")) {
      ...
      if (this.SPEED_LIMIT in this.captions) {
        ...
        (speedLimitCaption = this.captions[this.SPEED_LIMIT] = this.create()).classList.add("speed-warning", "transitionCaption");
      ...
      (speedLimitCaption = this.captions[this.SPEED_LIMIT]).querySelector("#slSubT > b").textContent = PPSLimit.toFixed(1);
      if (this.speedTimout) {
